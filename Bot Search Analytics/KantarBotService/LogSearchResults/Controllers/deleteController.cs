using System;
using System.Collections.Generic;
using System.Data;
using System.Data.Entity;
using System.Linq;
using System.Threading.Tasks;
using System.Net;
using System.Web;
using System.Web.Mvc;
using LogSearchResults.Models;

namespace LogSearchResults.Controllers
{
    public class deleteController : Controller
    {
        private KantarBotContext db = new KantarBotContext();

        // GET: delete
        public async Task<ActionResult> Index()
        {
            return View(await db.SearchLog.ToListAsync());
        }

        // GET: delete/Details/5
        public async Task<ActionResult> Details(int? id)
        {
            if (id == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
            }
            SearchLog searchLog = await db.SearchLog.FindAsync(id);
            if (searchLog == null)
            {
                return HttpNotFound();
            }
            return View(searchLog);
        }

        // GET: delete/Create
        public ActionResult Create()
        {
            return View();
        }

        // POST: delete/Create
        // To protect from overposting attacks, please enable the specific properties you want to bind to, for 
        // more details see http://go.microsoft.com/fwlink/?LinkId=317598.
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Create([Bind(Include = "Id,instance,username,keyword,searchtype")] SearchLog searchLog)
        {
            if (ModelState.IsValid)
            {
                db.SearchLog.Add(searchLog);
                await db.SaveChangesAsync();
                return RedirectToAction("Index");
            }

            return View(searchLog);
        }

        // GET: delete/Edit/5
        public async Task<ActionResult> Edit(int? id)
        {
            if (id == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
            }
            SearchLog searchLog = await db.SearchLog.FindAsync(id);
            if (searchLog == null)
            {
                return HttpNotFound();
            }
            return View(searchLog);
        }

        // POST: delete/Edit/5
        // To protect from overposting attacks, please enable the specific properties you want to bind to, for 
        // more details see http://go.microsoft.com/fwlink/?LinkId=317598.
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Edit([Bind(Include = "Id,instance,username,keyword,searchtype")] SearchLog searchLog)
        {
            if (ModelState.IsValid)
            {
                db.Entry(searchLog).State = EntityState.Modified;
                await db.SaveChangesAsync();
                return RedirectToAction("Index");
            }
            return View(searchLog);
        }

        // GET: delete/Delete/5
        public async Task<ActionResult> Delete(int? id)
        {
            if (id == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
            }
            SearchLog searchLog = await db.SearchLog.FindAsync(id);
            if (searchLog == null)
            {
                return HttpNotFound();
            }
            return View(searchLog);
        }

        // POST: delete/Delete/5
        [HttpPost, ActionName("Delete")]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> DeleteConfirmed(int id)
        {
            SearchLog searchLog = await db.SearchLog.FindAsync(id);
            db.SearchLog.Remove(searchLog);
            await db.SaveChangesAsync();
            return RedirectToAction("Index");
        }

        protected override void Dispose(bool disposing)
        {
            if (disposing)
            {
                db.Dispose();
            }
            base.Dispose(disposing);
        }
    }
}
