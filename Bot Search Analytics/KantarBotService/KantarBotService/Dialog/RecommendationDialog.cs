...
using KantarBotService.Support;
...
namespace KantarBotService.Dialog
{
    ...
    public class RecommendationDialog : IDialog<object>
    {
        ...
        private IForm<RecommendationData> BuildHotelsForm()
        {
            OnCompletionAsyncDelegate<RecommendationData> processHotelsSearch = async (context, state) =>
            {
            ...
            return new FormBuilder<RecommendationData>()
                .Field(nameof(RecommendationData.Destination))
        ...
        private async Task ResumeAfterHotelsFormDialog(IDialogContext context, IAwaitable<RecommendationData> result)
        {
            try
            {
        ...
        private async Task<IEnumerable<Recommendation>> GetHotelsAsync(RecommendationData searchQuery)
        {
            var hotels = new List<Recommendation>();

            // Filling the hotels results manually just for demo purposes
            for (int i = 1; i <= 5; i++)
            {
                var random = new Random(i);
                Recommendation hotel = new Recommendation()
                {
