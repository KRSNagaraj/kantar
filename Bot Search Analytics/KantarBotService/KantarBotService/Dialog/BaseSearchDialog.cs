...
{
    [Serializable]
    public class BaseSearchDialog : IDialog<object>
    {
        ...
        {
            context.Wait(this.MessageProcessAsync);
            //await context.("Thank you");
            //context.Call(this.MessageProcessAsync);
        }

        public async virtual Task MessageProcessAsync(IDialogContext context, IAwaitable<IMessageActivity> result)
        {
            await context.PostAsync("Pls. enter the search key word");
            var message = await result;
            var txt = context.UserData.ToString();
            if (string.Compare( message.Text, "back", true) == 0)
            {
                context.Done(1);
                ...
                await context.PostAsync($"Please type key words you want to search...");
